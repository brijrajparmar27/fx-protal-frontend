...
class RiskRadar extends React.Component {
  ...
  getXeroInvoices = async () => {
    ...
    if (res) {
      if (res.data.errorCode) {
        ...
        const invoices = this.parseXeroInvoices(res.data);
        // const invoices = [{
        //         "invoiceType": "ACCPAY",
  ...
  parseXeroInvoices = (invoices) => {
    let isAnySameRecord = false,
      isAnyModifyRecord = false;
    let updatedInvoices = [],
      newList = [],
      modifiedList = [],
      repeatedList = [];
            ...
            case 'ACCREC':
              {
                ...
                if (filteredRisks && filteredRisks.length > 0) {
                  ...
                  filteredRisks.forEach((fr) => {
                    if (fr.amount == inv.amountDue && fr.currencyCode == inv.currencyCode && fr.description == inv.description && fr.referenceId == inv.invoiceNumber) {
                      status = 'EXISTING';
                ...
                updatedInvoices = [...updatedInvoices, { ...inv, status }];
              }
              break;
            case 'ACCPAY':
              {
  ...
  riskRadarUploadNowInvoice = (selectedInvoice) => {
    ...
    if (invoiceToUpload && invoiceToUpload.length > 0) {
      ...
      if (modifiedList.length > 0 || repeatedList.length > 0) {
        this.setState({
          uploadXeroConfirmationModal: true,
          uploadXeroConfirmationModalHeader: 'Xero Invoice Upload',
          uploadXeroConfirmationModalMsg:
            'Only new records will be added to Risk Radar. Modified records will replace existing records in Risk Radar. If you would not like modified records to replace existing records, please press No in this message and Unselect before Uploading again.',
        ...
        invoiceToUpload.forEach(async (inv, index) => {
          if (inv && inv.invoiceType) {
            await this.addUpdateCategoryDetails(
              ...
              inv.currencyCode,
              inv.description,
              inv.invoiceType === 'ACCPAY' ? inv.plannedPaymentDate : inv.expectedPaymentDate,
              '',
              '',
              '',
              '',
  ...
  handleUploadXeroNegativeResponse = () => {
    // if (this.state.newXeroUploadList && this.state.newXeroUploadList.lemgth > 0)
  ...
  handleUploadXeroPositiveResponse = () => {
    if (this.state.newXeroUploadList && this.state.newXeroUploadList.length > 0) this.riskRadarUploadNewInvoice(this.state.newXeroUploadList);
    if (this.state.modifiedXeroUploadList && this.state.modifiedXeroUploadList.length > 0) this.riskRadarUploadNewInvoice(this.state.modifiedXeroUploadList);
    this.setState(
      {
        uploadXeroConfirmationModal: false,
        uploadXeroConfirmationModalHeader: '',
  ...
  riskRadarUploadLaterInvoice = (selectedInvoice) => {
    ...
    invoiceToUpload &&
      invoiceToUpload.forEach((inv, index) => {
        if (inv && inv.invoiceType) {
          let obj = {};
  ...
  parseRiskRadarData = (data, pastData, functionalCurrency, tabValue) => {
    ...
    if (data && data.risks && data.risks.length > 0) {
      data.risks.forEach((risk) => {
        // handle CASH Assets
        if (risk.riskType === 'ASSETS') {
          if (risk.notExpired) risk.dueDate = new Date();
        }
        tableObj = [];
        ...
        //   ...risk
        // });
        if (RISK_CATEGORY_INFO[type].type === 'ASSETS') {
          tableObj = [
            parsedRiskRadarData.risks[key + 'Display'].length,
            risk.referenceId,
            formatMoney(risk.amount) + ' ' + risk.currencyCode,
            formatDate(risk.notExpired ? new Date() : risk.dueDate),
            risk.description,
            risk.notExpired ? 'CASH' : '',
  ...
  combineRiskRadarData = (riskRadarData, pastData) => {
    ...
    if (pastData && pastData.risks && pastData.risks.length > 0) {
      pastData.risks.forEach((risk) => {
        ...
        } else if (risk.riskType === 'ASSETS') {
          updatedRiskRadarData.assets = [...updatedRiskRadarData.assets, risk];
        } else if (risk.riskType === 'LIABILITIES') {
          updatedRiskRadarData.liabilities = [...updatedRiskRadarData.liabilities, risk];
        }
      });
    }
    // Combine Hedges
  ...
  editRow = (name, index, obj) => {
    ...
    // let lists = this.state.riskRadarFullData && this.state.riskRadarFullData.risks[name] ? this.state.riskRadarFullData.risks[name] : [];
    let listsDisplay = this.state.riskRadarFullData && this.state.riskRadarFullData.risks[nameDisplay] ? this.state.riskRadarFullData.risks[nameDisplay] : [];

    let nodeDisplay = listsDisplay.find((row) => row[0] == index);
    let lists = this.state.riskRadarFullData && this.state.riskRadarFullData.risks[name].filter((n) => n.id === obj.id);
    let node = lists[0];
    // let node = lists[index];
    console.log('NODE - ', name, index, node);
    // Set state Object
    if (nodeDisplay && node) {
      let editObject = {};
  ...
  getDeleteRowIcon = (category, index, obj) => {
    return (
      <>
        <Edit onClick={() => this.editRow(category, index, obj)} className={cx(this.props.classes.editIcon, this.props.classes.icon)} />
  ...
  addUpdateCategoryDetails = async (
    ...
    console.log('calladdupdateapi - ', selectedCategory, isLastRecord, notExpired, this.state.callInProgress);
  ...
  closeGetStartedModal = () => {
    this.setState({ getStartedModal: false, getStartedModal2: false });
  };
  ...
  handleGetStarted2 = (event) => {
    this.setState({ getStartedModal2: true });
  };
        ...
        <GridItem xs={11} sm={11} md={11} lg={11}>
          <GridContainer>
            <GridItem xs={12} sm={12} md={12} lg={12}>
              ...
              <Tooltip
                ...
                title={
                  <>
                    ...
                    <span> to get additional information.</span>
                  </>
                }
                placement="top"
              ...
              <Button
                onClick={(e) => this.handleGetStarted2(e)}
                style={{
                  color: 'white',
                  backgroundColor: '#2391d2',
                  marginLeft: 15,
            ...
            <GridItem xs={12} sm={12} md={12} lg={12}>
              <div>
                ...
                />
                {this.state.uploadXeroConfirmationModal && (
                  <ConfirmationModal
                ...
                {this.state.getStartedModal && (
                  <GetStartedModal
                    ...
                    videoLink={'https://fxguard-cms.s3.eu-west-2.amazonaws.com/cms/public/images/Risk+Radar+Video+1.mp4'}
                  />
                )}
                {this.state.getStartedModal2 && (
                  <GetStartedModal
                    showModal={this.state.getStartedModal2}
                    closeModal={this.closeGetStartedModal}
                    title={'RISK RADAR VIDEO 2'}
                    videoLink={'https://fxguard-cms.s3.eu-west-2.amazonaws.com/cms/public/images/Risk+Radar+Video+Part+2.mp4'}
                  />
                )}
              </div>
            </GridItem>
