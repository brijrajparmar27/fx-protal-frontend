...
class ManageAccount extends React.Component {
  ...
  getRiskPlanUserList = async (user) => {
    if (this.state.role !== 'role-prospect-user' && this.state.role !== 'role-prospect' & (sessionStorage.getItem('ht') && this.state.role !== 'role-customer-manager')) {
      ...
      if (res.data.errorCode) {
        ...
        this.setState({ riskUsers: [...res.data.customerUsers], canInviteUsers: existingUserCount < maxUserAllowed - 1 });
      }
    }
  };
  ...
  reactivateSameRiskPlan = async () => {
    ...
    if (res.data.errorCode) {
      ...
      this.setState({
        noticeModal: true,
        noticeModalHeader: 'Information',
        noticeModalErrMsg: 'You have successfully reactivated your existing plan.',
      });
      this.getRiskPlanInfo();
    }
  };
  validateRiskEmail = () => {
    ...
    const riskUser =
      ...
      this.state.riskUsers.filter((userRow) => {
        // console.log(userRow);
        return this.state.inviteRiskEmail.toLowerCase() === userRow.email.toLowerCase();
      });
    ...
    if (this.state.inviteRiskEmailState === 'success' && this.state.inviteRiskUserRoleState === 'success') return true;
      ...
      if (res.data.errorCode) {
        if (res.data.errorCode === 401) {
          ...
          this.setState({
            noticeModal: true,
            noticeModalHeader: 'Error',
            noticeModalErrMsg: 'You do not have the permission to Invite users. Please contact your Admin.',
          });
        } else {
          this.setState({
            noticeModal: true,
  ...
  handleinviteRiskUserRole = (event) => {
    this.setState(validate(event.target.value, 'inviteRiskUserRole', this.state, [{ type: 'required' }], this.error));
  };
  handleRiskToggle = (event) => {
    this.setState({
      confirmationModal: true,
      confirmationModalHeader: 'Confirm',
      confirmationModalMsg: event.target.checked ? 'Are you sure you want to Enable the User' : 'Are you sure you want to Disable the User',
        ...
        <>
          {isRegistered ? (
            <FormControlLabel
              ...
              control={
                <Switch color="primary" tabIndex={-1} id={'checkbox_' + userID} onChange={this.handleRiskToggle} disabled={this.state.viewClient === 'true'} checked={enabled} />
              }
              label={<div />}
            />
          ) : (
            <a rel="noopener noreferrer" onClick={() => this.resendRiskInviteLink(emailAddress)} style={{ cursor: 'pointer' }}>
              Resend Link
            </a>
          )}
        </>
      ...
      this.state.riskUsers.forEach((userRow) => {
        if (userRow.id) {
          ...
          let user = [
            ...
            userRow.companyName,
            userRow.phoneNumber,
            userRow.role === 'role-prospect-user' || userRow.role === 'role-prospect' ? 'Risk Main User' : userRow.role === 'role-prospect-invite-user' ? 'Risk User' : '',
            fillButtons(userRow.id, userRow.email, userRow.enabled, userRow.firstName && userRow.phoneNumber),
          ];
          users.push(user);
        }
      });
    ...
    const fillButtons = (userID, userData, isRegistered) => {
      return (
        ...
        /> */}
          ...
          />
          {isRegistered ? (
            <Edit onClick={this.handleEditUser.bind(this, userData)} className={cx(this.props.classes.editIcon, this.props.classes.icon)} />
          ) : (
            <a rel="noopener noreferrer" onClick={() => this.resendInviteLink(userData.emailAddress)} style={{ cursor: 'pointer' }}>
              Resend Link
            </a>
          )}
        </>
      ...
      this.state.users.forEach((userRow) => {
        if (userRow.id) {
          ...
          let user = [
            ...
            userRow.companyName,
            userRow.phoneNumber,
            userRow.role === 'role-customer-user' ? 'Customer User' : userRow.role === 'role-customer-user-manager' ? 'Customer Manager' : '',
            fillButtons(
              userRow.id,
              {
                id: userRow.id,
  ...
  saveUser = async () => {
    const userData = {
      ...
      phoneNumber: this.state.countryCode + this.state.phoneNumber.replace(/-/g, ''),
      username: this.state.email,
      currentPassword: this.state.currentPassword !== '' ? this.state.currentPassword : null,
      ...
      if (res.data.errorCode) {
        if (res.data.errorCode === 401) {
          ...
          if (this.state.newPassword !== '') {
            errMsg = 'There is some error in updating Password. Please check if current password is correct';
          } else {
  ...
  validatePassword = () => {
    ...
    if (this.state.currentPasswordState !== 'success') {
      states = {
        ...states,
        ...validate(this.state.currentPassword, 'currentPassword', this.state, this.rulesList.currentPassword, this.error),
      };
    }
    if (this.state.newPasswordState !== 'success') {
      states = {
        ...states,
        ...validate(this.state.newPassword, 'newPassword', this.state, this.rulesList.newPassword, this.error),
      };
    }
    if (this.state.confirmNewPasswordState !== 'success') {
      states = {
        ...states,
        ...validate(this.state.confirmNewPassword, 'confirmNewPassword', this.state, this.rulesList.confirmNewPassword, this.error),
      };
    }
    ...
    if (states.currentPasswordState === 'success' && states.newPasswordState === 'success' && states.confirmNewPasswordState === 'success') {
      return true;
    } else {
      return false;
    }
  ...
  isValidated = () => {
    ...
    if (this.state.firstNameState !== 'success') {
      states = {
        ...states,
        ...validate(this.state.firstName, 'firstName', this.state, this.rulesList.firstName, this.error),
      };
    }
    if (this.state.lastNameState !== 'success') {
      states = {
        ...states,
        ...validate(this.state.lastName, 'lastName', this.state, this.rulesList.lastName, this.error),
      };
    }
    if (this.state.emailState !== 'success') {
      states = {
        ...states,
        ...validate(this.state.email, 'email', this.state, this.rulesList.email, this.error),
      };
    }
    if (this.state.phoneState !== 'success') {
      states = {
        ...states,
        ...validate(this.state.phoneNumber.replace(/-/g, ''), 'phone', this.state, this.rulesList.phone, this.error),
      };
    }
    if (this.state.companyNameState !== 'success') {
      states = {
        ...states,
        ...validate(this.state.companyName, 'companyName', this.state, this.rulesList.companyName, this.error),
      };
    }
    if (this.state.countryCodeState !== 'success') {
      states = {
        ...states,
        ...validate(this.state.countryCode, 'countryCode', this.state, this.rulesList.country, this.error),
      };
    }
  ...
  upgradePlan = async (planInfo) => {
    if (this.state.role === 'role-prospect') {
      this.setState({
        ...
        noticeModalErrMsg: 'You cannot upgrade the plan in the Demo mode. In Live mode, the Plan can be updated.',
      });
      return;
    }
    ...
    if (res.data.errorCode) {
      if (res.data.errorCode === 401) {
        ...
        this.setState({
          noticeModal: true,
          noticeModalHeader: 'Error',
          noticeModalErrMsg: 'You do not have the permission to UPGRADE. Please contact your Admin.',
        });
      } else {
        this.setState({
          noticeModal: true,
  ...
  handleUpgradePlanClick = async () => {
    ...
    const res = await apiHandler({
      url: endpoint.PLAN_UPGRADE + '?subscribedPlanId=' + this.state.subscribedPlanId,
      authToken: sessionStorage.getItem('token'),
    });
  ...
  getPlanVolumeOptions = async (planOptions) => {
    if (planOptions.length > 0) {
      ...
      const res = await apiHandler({
        url: endpoint.PLAN_FIND + '?id=' + planId + '&customerId=' + this.state.customerId,
        authToken: sessionStorage.getItem('token'),
      });
      this.setState({ callInProgress: false });
      if (res.data.errorCode) {
  ...
  validateEmail = () => {
    if (this.state.inviteEmailState === 'success' && this.state.inviteUserRoleState === 'success') return true;
  ...
  handleInviteUserRole = (event) => {
    this.setState(validate(event.target.value, 'inviteUserRole', this.state, [{ type: 'required' }], this.error));
  };
    ...
    return (
      <>
        <GridContainer justify="center">
                        ...
                        <GridContainer>
                          ...
                          <GridItem xs={12} sm={12} md={4} lg={4}>
                            <CustomInput
                              ...
                              helpText={this.state.emailAddressState === 'error' && this.state.emailAddressErrorMsg[0]}
                          ...
                          {this.state.viewClient !== 'true' && (
                            <>
                              <GridItem xs={10} sm={10} md={12} lg={12} className={classes.alignPadding}>
                                Change Password
                              </GridItem>
                              <GridItem xs={10} sm={10} md={4} lg={4} className={classes.alignPadding}>
                                <CustomInput
                                  success={this.state.currentPasswordState === 'success'}
                                  error={this.state.currentPasswordState === 'error'}
                                  helpText={this.state.currentPasswordState === 'error' && this.state.currentPasswordErrorMsg[0]}
                                  ...
                                  formControlProps={{
                                    ...
                                    onBlur: (event) => {
                                      ...
                                      this.change(event, 'currentPassword', [{ type: 'required' }]);
                                    },
                                    onChange: (event) => {
                              ...
                              <GridItem xs={10} sm={10} md={4} lg={4} className={classes.alignPadding}>
                                <CustomInput
                                  ...
                                  helpText={this.state.newPasswordState === 'error' && this.state.newPasswordErrorMsg[0]}
                              ...
                              <GridItem xs={10} sm={10} md={4} lg={4} className={classes.alignPadding}>
                                <CustomInput
                                  ...
                                  helpText={this.state.confirmNewPasswordState === 'error' && this.state.confirmNewPasswordErrorMsg[0]}
                                  labelText="New Password Confirmation*"
                                  id="ma_confirmNewPassword"
                                  inputProps={{
                                    value: this.state.confirmNewPassword,
                                    type: 'password',
                                    onChange: (event) => this.handleChange('confirmNewPassword', event),
                                  }}
                                  formControlProps={{
                                    fullWidth: true,
                                    className: classes.customFormControlClasses,
                                    ...
                                    onChange: (event) => {
                                      if (!this.state.confirmNewPasswordPristine) {
                                        this.setState({
                                          confirmNewPasswordPristine: false,
                                        });
                                        this.change(event, 'confirmNewPassword', [
                              ...
                              <GridItem xs={12} sm={12} md={12} lg={12}>
                                <div className={classes.center} style={{ textAlign: 'right' }}>
                                  <Button round={false} color="info" size="lg" onClick={() => this.saveUser()}>
                                    SAVE
                                  </Button>
                                </div>
                              </GridItem>
          ...
          {(sessionStorage.getItem('module') === 'BOTH' || sessionStorage.getItem('module') === 'TRANSACTION') && (
          <GridItem xs={12} sm={12} md={11} lg={11}>
          <GridContainer>
          <GridItem xs={12} sm={12} md={10} lg={10}>
            <h4>
              ...
              <CardBody>
                ...
                {this.state.subscribedPlanId === 1 && (
                  <>
                    ...
                    <p>
                      Eliminate extra fees on your operation by joining our
                      subscription plan
                    </p>
                  </>
                )}
                {this.state.subscribedPlanId === 2 && (
                  <>
                    ...
                    <div className={classes.detailsRow}>
                      <span className={classes.floatLeft}>Plan Start Date</span>
                      <span>
                        {formatDate(this.state.subscriptionPlanStartDate)}
                      </span>
                    </div>
                    <div className={classes.detailsRow}>
                      <span className={classes.floatLeft}>Plan End Date</span>
                      <span>
                        {formatDate(this.state.subscriptionPlanExpiryDate)}
                      </span>
                    </div>
                    <div className={classes.detailsRow}>
                      <span className={classes.floatLeft}>Limit Consumed</span>
                      <span>
                        {(
                          (+this.state.volumeLimitUsed / +this.state.volume) *
                          100
                        ).toFixed(2)}
                        {""}
                        {"%"}
          ...
          <Card>
                    <CardHeader color="warning" text>
                      ...
                      <span className={classes.title}> Transaction Management Users </span>
                    </CardHeader>
                    <CardBody>
                          ...
                          <GridItem xs={12} sm={12} md={12} lg={12}>
                            <GridContainer justify="flex-end">
                              ...
                              <GridItem xs={12} sm={12} md={6} lg={4}>
                                <CustomInput
                                  success={this.state.inviteEmailState === 'success'}
                                  error={this.state.inviteEmailState === 'error'}
                                  helpText={this.state.inviteEmailState === 'error' && this.state.inviteEmailErrorMsg[0]}
                                  labelText="Email address"
                                  id="ma_inviteEmail"
                                  inputProps={{
                                    value: this.state.inviteEmail,
                                    onChange: (event) => this.handleChange('inviteEmail', event),
                                  }}
                                  formControlProps={{
                                    fullWidth: true,
                                    ...
                                    onBlur: (event) => {
                                      ...
                                      this.change(event, 'inviteEmail', [{ type: 'required' }, { type: 'email' }]);
                                    },
                                    onChange: (event) => {
                              ...
                              <GridItem xs={12} sm={12} md={4} lg={4}>
                                <FormControl fullWidth className={classes.filledSelect} style={{ marginTop: -25 }}>
                                  <FormHelperText className={classes.selectFormHelperText}>Role</FormHelperText>
                                  <Select
                                    MenuProps={{
                                      className: classes.selectMenu,
                                    }}
                              ...
                              <GridItem xs={12} sm={12} md={12} lg={2}>
                                <Button round={false} color="info" size="md" style={{ lineHeight: '10px' }} onClick={this.inviteUser.bind(this)}>
                                  invite
                                </Button>
                              </GridItem>
                            </GridContainer>
                      ...
                      <GridItem xs={12} sm={12} md={12} lg={12}>
                        <Table
                          tableHead={['First Name', 'Last Name', 'E-mail', 'Company Name', 'Phone Number', 'Actions']}
                      ...
                      <GridItem xs={12} sm={12} md={12} lg={12} style={{ textAlign: 'center' }}>
                        <EditUsers handleClose={this.closeUserEditModal} showModal={this.state.showEditUser} saveUserRole={this.saveUserRole} userData={this.state.userData} />
                      </GridItem>
                    </CardBody>
                  </Card>
                )}
          ...
          {(sessionStorage.getItem('module') === 'BOTH' || sessionStorage.getItem('module') === 'RISKS') && (
          <GridItem xs={12} sm={12} md={11} lg={11}>
          <GridContainer>
          <GridItem xs={12} sm={12} md={10} lg={10}>
            <h4>
          ...
          <Card>
              <CardHeader color="warning" text>
                ...
                <span className={classes.title}>FX RISK MANAGEMENT PLAN</span>
              </CardHeader>
              <CardBody>
                {this.state.riskStatus && (
                  <>
                    <h4>
                      <strong>{'Your selected FX Risk Management Plan'}</strong>
                    </h4>
                    {this.state.riskStatus.description && this.state.riskStatus.description.split('\r\n').map((d, i) => <p key={i}>{d}</p>)}
                    <>
                      <div className={classes.detailsRow}>
                        <span className={classes.floatLeft}>Max Users</span>
                        <span>{this.state.riskStatus.maxUserAllowed}</span>
                      </div>
                      <div className={classes.detailsRow}>
                        <span className={classes.floatLeft}>Currency</span>
                        <span>{this.state.riskStatus.currency}</span>
                      </div>
                      <div className={classes.detailsRow}>
                        <span className={classes.floatLeft}>Plan Start Date</span>
                        <span>{formatDate(this.state.riskStatus.planStartDate)}</span>
                      </div>
                      <div className={classes.detailsRow}>
                        <span className={classes.floatLeft}>Plan End Date</span>
                        <span>{formatDate(this.state.riskStatus.planExpiryDate)}</span>
                      </div>
                      {this.state.riskStatus.cardDetailExists && (
                        <>
                      <div className={classes.detailsRow}>
                        <span className={classes.floatLeft}>Frequency</span>
                        <span>{this.state.riskStatus.planFrequency}</span>
                      </div>
                      {(this.state.role === 'role-prospect-user' || this.state.role === 'role-prospect' || (sessionStorage.getItem('ht') && this.state.role === 'role-customer-manager')) && (
                        <>
                          <div className={classes.detailsRow}>
                            <span className={classes.floatLeft}>Card last 4 digits</span>
                            <span>{this.state.riskStatus.last4Digit}</span>
                          </div>
                          <div className={classes.detailsRow}>
                            <span className={classes.floatLeft}>Card Expiry (MM-YYYY)</span>
                            <span>{this.state.riskStatus.expMonth + ' - ' + this.state.riskStatus.expYear}</span>
                          </div>
                        </>
                      )}
                      </>
                      ...
                      <>
                        {this.state.riskStatus.subsCancelled ? (
                          <Button
                            ...
                            onClick={() => this.reactivateRiskPlanSubscription()}
                          >
                            Reactivate Plan
                          </Button>
                        ) : (
                          ...
                          {this.state.riskStatus.cardDetailExists && (
                            <Button
                              ...
                              onClick={() => this.changeRiskPlanSubscription()}
                            >
                              CHANGE PLAN
                            </Button>
                            )}
          ...
          <GridItem xs={12} sm={12} md={8} lg={8}>
          {(this.state.role === 'role-prospect' || this.state.role === 'role-prospect-user' || (sessionStorage.getItem('ht') && this.state.role === 'role-customer-manager')) && (
          <Card>
                    <CardHeader color="warning" text>
                      <CardText color="warning">
                        <Work className={classes.listItemIcon} />
                      </CardText>
                      <span className={classes.title}> Risk Management Users </span>
                    </CardHeader>
                    <CardBody>
                      <div style={{ textAlign: 'right' }}>
                        {(this.state.role === 'role-prospect' || this.state.role === 'role-prospect-user' || (sessionStorage.getItem('ht') && this.state.role === 'role-customer-manager')) && this.state.canInviteUsers && (
                          <GridItem xs={12} sm={12} md={12} lg={12}>
                            <GridContainer justify="flex-end">
                              <span style={{ marginTop: 15 }}>Invite User</span>
                              <GridItem xs={12} sm={12} md={6} lg={4}>
                                <CustomInput
                                  success={this.state.inviteRiskEmailState === 'success'}
                                  error={this.state.inviteRiskEmailState === 'error'}
                                  helpText={this.state.inviteRiskEmailState === 'error' && this.state.inviteRiskEmailErrorMsg[0]}
                                  labelText="Email address"
                                  id="ma_inviteRiskEmail"
                                  inputProps={{
                                    value: this.state.inviteRiskEmail,
                                    onChange: (event) => this.handleChange('inviteRiskEmail', event),
                                  }}
                                  formControlProps={{
                                    fullWidth: true,
                                    className: classes.filterInput,
                                    onBlur: (event) => {
                                      ...
                                      this.change(event, 'inviteRiskEmail', [{ type: 'required' }, { type: 'email' }]);
                                    },
                                    onChange: (event) => {
                                      if (!this.state.inviteRiskEmailPristine) {
                                        this.setState({
                                          inviteRiskEmailPristine: false,
                                        });
                                        this.change(event, 'inviteRiskEmail', [{ type: 'required' }, { type: 'email' }]);
                              ...
                              <GridItem xs={12} sm={12} md={12} lg={2}>
                                <Button round={false} color="info" size="md" style={{ lineHeight: '10px' }} onClick={this.riskInviteUser.bind(this)}>
                                  invite
                                </Button>
                              </GridItem>
                            </GridContainer>
        ...
        >
          ...
          <DialogContent id="notice-modal-slide-description" className={classes.modalBody}>
            <p>{this.state.noticeModalErrMsg === '' ? "Please click the 'commercial terms & conditions' to submit" : this.state.noticeModalErrMsg}</p>
          </DialogContent>
        ...
        {this.state.showManageSubscriptionModal && (
          <ManageSubscription showModal={this.state.showManageSubscriptionModal} closeModal={this.closeManageSubscriptionModal} planDetails={this.state.riskStatus} />
        )}
