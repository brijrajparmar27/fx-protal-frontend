...
import CreateSARRecord from "views/Components/CreateSARRecord.jsx";
...
const Columns = ["Client ID", "Client Name", "Report Date", "Reason"];
...
class SARReport extends React.Component {
  ...
  getSARReport = async () => {
    ...
    // ADMIN_SARREPORT_FIND
    const res = await apiHandler({
      url: endpoint.ADMIN_SARREPORT_FIND + queryParam,
      authToken: sessionStorage.getItem("token")
    });
  ...
  parseIndicatorData = data => {
    ...
    data.tasks &&
      data.tasks.forEach(sar => {
        let row = [
          sar.id,
          sar.customerId,
          sar.customerName,
          formatDate(sar.reportDate),
          sar.reason
        ];
        reportData.push(row);
        downloadData.push([
  ...
  handleTableRowClick = async selectedID => {
    //ADMIN_SARREPORT_GET
    const res = await apiHandler({
      url: endpoint.ADMIN_SARREPORT_GET + selectedID,
      authToken: sessionStorage.getItem("token")
    });
    ...
    if (res.data.errorCode) {
      ...
      if (res.data.documentLinks.length) {
        res.data.documentLinks.forEach(doc => {
          const newdoc = doc.substring(1, doc.length - 1);
          doc = newdoc;
        });
      }
      this.setState({
        showRecordDlg: true,
        recordDetails: res.data
      });
    }
  };
  downloadReport = () => {
  ...
  closeCreateSARDialog = () => {
    this.setState({ showCreateSARDialog: false });
  };
  closeRecordModal = () => {
    this.setState({ showRecordDlg: false, recordDetails: {} });
  };
  addRecord = async record => {
    // ADMIN_SARREPORT_CREATE
    const res = await apiHandler({
      url: endpoint.ADMIN_SARREPORT_CREATE,
      method: "POST",
      authToken: sessionStorage.getItem("token"),
      data: record
    });
    ...
    return (
      <GridContainer justify="center">
        <GridItem xs={12} sm={12} md={11} lg={11}>
                  ...
                  <GridContainer justify="flex-end">
                    ...
                    >
                      <Button
                        ...
                        onClick={() => {
                          this.setState({ showCreateSARDialog: true });
                        }}
                      >
                        <AddIcon />
                        ADD
          ...
          <CreateSARRecord
            showModal={this.state.showCreateSARDialog}
            closeModal={this.closeCreateSARDialog}
...
export default withRouter(withStyles(styles)(SARReport));
