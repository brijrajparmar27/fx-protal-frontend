...
const Columns = ["Date", "Email"];

class KeepmeUpdatedReport extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount = () => {
    this.getKeepmeUpdatedReport();
  };

  getKeepmeUpdatedReport = async () => {
    let fromDate = "";
    let toDate = "";
    let queryParam = "";
    // Check for Filter
    ...
    const res = await apiHandler({
      url: endpoint.ADMIN_UPDATE_FIND + queryParam,
    ...
    if (res.data.errorCode) {
      ...
      this.setState({
        apiData:
          res.data && res.data.keepMeUpdated ? res.data.keepMeUpdated : [],
        columnSortKey: ""
      });
  ...
  parseIndicatorData = data => {
    ...
    data.keepMeUpdated &&
      data.keepMeUpdated.forEach((keep, index) => {
        let row = [index, formatDate(keep.createdDate), keep.email];
        reportData.push(row);
        downloadData.push([formatDate(keep.createdDate), keep.email]);
      });
  ...
  onClickColumnHeader = columnDetails => {
    let arr = sortArray(
      this.state.apiData,
      columnDetails.key,
...
KeepmeUpdatedReport.propTypes = {
  classes: PropTypes.object.isRequired
};

export default withRouter(withStyles(styles)(KeepmeUpdatedReport));
