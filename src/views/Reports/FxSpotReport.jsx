...
class FxSpotReport extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleStatus = async (activityId, event) => {
    ...
    const res = await apiHandler({
      url:
        endpoint.ADMIN_ACTIVITY_DETAIL_TRADE +
  ...
  getFxSpotReport = async () => {
    ...
    //?type=FXSPOT
    const res = await apiHandler({
      url: endpoint.ADMIN_TRADE_FIND + "?type=FXSPOT" + queryParam,
      authToken: sessionStorage.getItem("token")
    });
  ...
  handleTableRowClick = async selectedID => {
    ...
    // DEAL
    const res = await apiHandler({
      url: endpoint.DEAL + "?type=FXSPOT&dealId=" + selectedID,
      authToken: sessionStorage.getItem("token")
    });
                          ...
                          >
                            ...
                            >
                              ...
                              <CSVLink
                                ...
                                filename={
                                  "FxSpot" + formatDate(Date.now()) + ".csv"
                                }
                                className="hidden"
...
FxSpotReport.propTypes = {
  classes: PropTypes.object.isRequired
};

export default withRouter(withStyles(styles)(FxSpotReport));
