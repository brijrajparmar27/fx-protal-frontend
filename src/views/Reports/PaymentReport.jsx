...
class PaymentReport extends React.Component {
  ...
  getPaymentReport = async () => {
    ...
    // ADMIN_TRADE_FIND
    const res = await apiHandler({
      url: endpoint.ADMIN_TRADE_FIND + "?type=PAYMENT" + queryParam,
      authToken: sessionStorage.getItem("token")
    });
    ...
    data.customerPaymentDetails &&
      data.customerPaymentDetails.forEach(pay => {
        let row = [
          ...
          pay.beneficiaryName,
          formatMoney(pay.amount) /*+ " " + pay.amountCurrencyCode*/,
  ...
  handleTableRowClick = async selectedID => {
    ...
    const res = await apiHandler({
      url: endpoint.DEAL + "?type=PAYMENT&dealId=" + selectedID,
      authToken: sessionStorage.getItem("token")
    });
    if (res.data.errorCode) {
      if (res.data.errorCode === 401) {
...
export default withRouter(withStyles(styles)(PaymentReport));
