...
class CustomDropdown extends React.Component {
  ...
  handleClickMenu(param) {
    console.log('handleCloseMenu', param);
    this.setState({ open: false });
    if (param && param.onClick) {
      param.onClick();
    }
  }
  render() {
      ...
      <MenuList role='menu' className={classes.menuList}>
        ...
        {dropdownList.map((prop, key) => {
          if (prop.divider) {
            ...
            return (
              <MenuItem key={key} onClick={() => this.handleCloseMenu(prop)} className={dropdownItem} style={{ margin: 0 }} size='sm'>
                {prop.text}
              </MenuItem>
            );
          } else if (prop.click) {
            return (
              <MenuItem key={key} onClick={() => this.handleClickMenu(prop)} className={dropdownItem} style={{ margin: 0 }} size='sm'>
                {prop.text}
              </MenuItem>
            );
          }
      ...
      <div className={innerDropDown ? classes.innerManager : classes.manager}>
        ...
        <Popper
          ...
          className={classNames({
            ...
            [classes.pooperResponsiveNew]: true,
